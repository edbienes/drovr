...
# Every LIVE room query MUST be scoped to OUR workspace via `drovr_panes` so we never resolve,
...
# drovr_workspace_id : echo THIS orchestrator's own workspace_id (derived from our pane).
...
# drovr_panes : `herdr pane list` SCOPED to our workspace only. Use this everywhere a live
...
# drovr_assert_ours <pane_id> : exit 0 iff <pane_id> belongs to OUR workspace.
# Builds the allowed set from drovr_panes (workspace-scoped), so a foreign pane id ALWAYS fails.
drovr_assert_ours() {
  local snap; snap="$(drovr_panes)"
  pane_in_list "$1" <<< "$snap"
}

# drovr_send <pane_id> <text...> : HARD-GUARDED send. Asserts the pane is in our
...
drovr_send() {
  local pid="$1"; shift
  drovr_assert_ours "$pid" || {
    printf 'drovr_send REFUSED: pane %s is not in our workspace (%s)\n' \
      "$pid" "$(drovr_workspace_id 2>/dev/null)" >&2
    return 3
  }
  herdr pane run "$pid" "$@" >/dev/null   # fire-and-forget; drop herdr's JSON ack (rc still propagates)
}

# drovr_send_prompt <pane_id> <text> : HARD-GUARDED delivery of a LONG prompt/trigger. Same workspace
# guard as drovr_send, but delivers the text and the submit Enter as TWO SEPARATE herdr requests
...
#   commands (/new,/exit) go through drovr_send_slash below — the bundled `pane run` Enter was believed
...
drovr_send_prompt() {
  local pid="$1"; shift
  drovr_assert_ours "$pid" || {
    printf 'drovr_send_prompt REFUSED: pane %s is not in our workspace (%s)\n' \
      "$pid" "$(drovr_workspace_id 2>/dev/null)" >&2
    return 3
  }
...
# drovr_send_slash <pane_id> </command> : DROPDOWN-PROOF delivery of a leading-slash TUI command
...
# delivery bug #2). Fix = drovr_send_prompt (send-text + discrete Enter) followed by a SECOND discrete
...
# double-fire — same safety argument as _drovr_fire's bare-Enter retry. Only for TUI slash commands;
# plain shell lines stay on drovr_send.
drovr_send_slash() {
  local pid="$1" cmd="$2"
  drovr_send_prompt "$pid" "$cmd" || return 3
  sleep 1
  drovr_assert_ours "$pid" && herdr pane send-keys "$pid" Enter >/dev/null
}
...
# _drovr_alive <pane_id> <label> : exit 0 iff the pane hosts a LIVE agent (not a bare/never-launched/
# crashed-to-zsh shell). DISTINCT from _drovr_busy: busy = "actively working" (liberal, false-busy is
...
# So we capture ONE snapshot via `snap="$(drovr_panes)"` then resolve with
...
#                         _drovr_set_ctx_impl BEFORE provision_role runs), so it re-applies on the
...
# _drovr_reset <pane_id> <label> : TASK-BOUNDARY context reset for a REUSED pane. grok: /new (its only
...
# repo root, relaunch:  /exit → cd "$root" → <_drovr_launch_for>. `cd "$root"` is QUOTED so a bitten
...
# transition, so the caller's _drovr_settle is a genuine wait again. Slash commands (/new, /exit) go
# through drovr_send_slash (dropdown-proof: send-text + Enter + idempotent Enter — the bundled `pane run`
# Enter was consumed by the slash-autocomplete dropdown live); plain shell lines stay on drovr_send.
...
_drovr_reset() {
  ...
  if [ "$label" = grok-pressure-test ]; then
    drovr_send_slash "$id" '/new' || return 3   # dropdown-proof (send-text + Enter + idempotent Enter)
    sleep 3   # head-start only; /new doesn't move agent-status — _drovr_fire confirms trigger pickup
    return 0
  fi
  ...
  drovr_send_slash "$id" '/exit'  || return 3   # leave the (possibly stale-worktree) Claude session — dropdown-proof
  sleep 4                                          # let the TUI quit back to the shell before typing cd
  drovr_send "$id" "cd \"$root\"" || return 3   # back to repo root — QUOTED (repo paths contain spaces)
  sleep 1
  drovr_send "$id" "$(_drovr_launch_for "$label")" || return 3   # relaunch (same contract as fresh launch)
  _drovr_settle "$id" 30000                 # real absent→idle wait (relaunch boots the agent)
}

# _drovr_settle <pane_id> [timeout_ms] : after a /clear|/new reset (or a fresh launch), block until
...
# _drovr_reusable <agent_status> : exit 0 iff a PRESENT pane in this status can be reset+reused
...
provision_role() {
  ...
  snap="$(drovr_panes)"   # SCOPED to our workspace — single-command subst (reliable)
  id="$(pane_id_for_label "$label" <<< "$snap" || true)"             # here-string, no $(|func)
  pstatus="$(pane_status_for_label "$label" <<< "$snap" 2>/dev/null || echo unknown)"

  if [ -n "$id" ] && _drovr_reusable "$pstatus"; then
    # Guarded task-boundary reset (grok: /new + head-start; claude: /exit→cd root→relaunch + real settle).
    # _drovr_fire still verifies trigger pickup and re-sends once if a fire races the reset.
    _drovr_reset "$id" "$label" || return 3
    printf '%s\n' "$id"; return 0
  fi
