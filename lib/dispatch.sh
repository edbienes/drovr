...
#     implementation arm, grok-build/composer-fast the escalation arms. See devloop_dispatch_impl /
#     DL_IMPL_AGENT.
...
#
# Every fire goes through devloop_send (workspace-ownership guard). NEVER `herdr pane run` a
...
_DEVLOOP_TMPL="$(dirname "${BASH_SOURCE[0]}")/../templates"
_DEVLOOP_LIB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"   # abs lib dir (for pane-side helpers like forge-pretrust.sh)
...
# _devloop_load_config : apply the target repo's .devloop/config — the repo's POLICY declaration
...
_DEVLOOP_CFG_VARS="DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_PLAN_FIRST DL_IMPL_AGENT DL_GROK_LENS DL_TIER"
_devloop_load_config() {
  local cfg="${DL_REPO_PATH:-}/.devloop/config" v val
  ...
  while IFS='=' read -r v val; do
    ...
    env -i bash -c '. "$1" 2>/dev/null; for v in '"$_DEVLOOP_CFG_VARS"'; do [ -n "${!v:-}" ] && printf "%s=%s\n" "$v" "${!v}"; done; true' _ "$cfg"
  )
  return 0
}

# _devloop_base_fetch <repo-path> <base> : echo the "git fetch -q <remote> <branch> && " prefix for a
...
# _devloop_set_ctx <task> : export the DL_* context for this repo + task. Used by BOTH dispatch and
# collect, so a reprompt on a fresh re-invoke (env not preserved) re-fills triggers identically.
# Needs cwd inside the repo (git rev-parse); a detached background shell should export DEVLOOP_REPO_SLUG.
_devloop_set_ctx() {
  export DL_TASK="$1"
  ...
  _devloop_load_config                     # repo policy (env > config > default; sandboxed)
...
# _devloop_fire <pid> <label> <trigger> : deliver a trigger via devloop_send_prompt (send-text + a SEPARATE
...
# whenever _devloop_busy false-negatived a genuinely-working agent — the second copy landing ~10s later, and
# the model answering both (observed live 2026-06-25). The bare Enter can never double-send; devloop_send_prompt
# already makes the first Enter submit reliably, so the rare genuine miss is caught downstream by
# devloop_collect_one's file-sentinel reprompt (the real backstop). The "is it working?" check routes through
# _devloop_busy — which for grok reads the rendered footer (`Ctrl+c:cancel`), NOT agent_status, because grok's
# agent_status false-reads 'idle' for sustained stretches mid-run.
# FAIL-CLOSED on a DEAD PANE (added 2026-07-06 — a prompt was typed into a bare shell and forgotten,
# TWICE 06-24/07-06): _devloop_alive gates the send (no live agent → refuse BEFORE the prompt lands in zsh)
...
_devloop_fire() {
  ...
  if ! _devloop_alive "$pid" "$label"; then
    echo "_devloop_fire: $label pane $pid has NO LIVE AGENT (bare/crashed shell?) — REFUSING to fire. Relaunch it (provision_role / _devloop_reset) and re-dispatch." >&2
    return 4
  fi
  devloop_send_prompt "$pid" "$trig" || return 3
  t=0
  while [ "$t" -lt 10 ]; do
    sleep 1; t=$((t+1))
    _devloop_busy "$pid" "$label" && return 0   # picked up (grok: rendered footer; claude: agent_status)
  done
  if ! _devloop_alive "$pid" "$label"; then
    echo "_devloop_fire: $label pane $pid lost its agent after send — trigger NOT DELIVERED. Relaunch and re-dispatch." >&2
    return 4
  fi
  echo "_devloop_fire: $label not detected working ~10s after send — discrete Enter (submit-pill guard, no re-send)" >&2
  devloop_assert_ours "$pid" && herdr pane send-keys "$pid" Enter >/dev/null   # submits a stuck pill; no-op on empty composer
  return 0
}

# devloop_dispatch_reviews <task> : write task.md, provision both reviewer panes, fire both PROSE
# triggers (guarded). Phase-1 MVP target = current branch vs main. Echoes the resolved pane ids.
devloop_dispatch_reviews() {
  _devloop_set_ctx "$1"
  _fill "$_DEVLOOP_TMPL/task.md.tmpl" | bus_write "$DL_TASK" task.md >/dev/null
  ...
  # $(_fill …) strips the trailing newline → each trigger is delivered as a single line. _devloop_fire
  # confirms pickup + retries once (a trigger fired right after provision's /clear can be swallowed).
  _devloop_fire "$rid" claude-code-review      "$(_fill "$_DEVLOOP_TMPL/trigger-review.txt")"   || { echo "dispatch: review trigger not delivered (workspace guard or DEAD PANE — see stderr)" >&2; return 3; }
  ...
  if [ "${DL_GROK_LENS:-pane}" = opencode ]; then
    ...
    bash "$_DEVLOOP_LIB/grok-review.sh" "$DL_TASK" reviews/grok.md "$DL_REPO_PATH" "$DL_TARGET" \
      || { echo "dispatch: grok-review helper failed" >&2; return 1; }
  else
    _devloop_fire "$gid" grok-pressure-test "$(_fill "$_DEVLOOP_TMPL/trigger-pressure.txt")" || { echo "dispatch: pressure trigger not delivered (workspace guard or DEAD PANE — see stderr)" >&2; return 3; }
  fi
  echo "dispatched (guarded): claude-code-review=$rid grok-pressure-test=$gid"
}

# devloop_collect_one <task> <label> <relpath> <trigger-text> : return 0 once <relpath> is complete
...
# is its own ceiling because devloop_collect_all runs on a DEADLINE re-invoke, AFTER the Task-7
...
# re-prompts through devloop_send only. 'pstatus' not 'status' — 'status' is a read-only special var in zsh.
# The "still working → wait, don't reprompt" check routes through _devloop_busy (grok: rendered footer;
# claude: agent_status), because grok's agent_status false-reads 'idle' for SUSTAINED stretches mid-run —
# the old `pstatus = working` test let a working grok skip the wait and get a spurious reprompt into a live
# run. bus_ready (the file sentinel) is still checked FIRST and is the authoritative completion signal.
devloop_collect_one() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    if _devloop_busy "$id" "$label"; then sleep 3; continue; fi   # still reviewing — wait, don't reprompt
    ...
    if [ -n "$id" ]; then
      ...
      devloop_send_prompt "$id" "$trigger" || return 3          # guarded reprompt (send-text + Enter)
      sleep 3
    else
...
# devloop_collect_all <task> : collect BOTH reviewers (reprompt-bounded each). Echoes one status line
...
devloop_collect_all() {
  local task="$1" rc=0
  _devloop_set_ctx "$task"
  if devloop_collect_one "$task" claude-code-review reviews/claude.md "$(_fill "$_DEVLOOP_TMPL/trigger-review.txt")"; then
    echo "claude-code-review OK"
  else echo "claude-code-review STALLED"; rc=1; fi
  if devloop_collect_one "$task" grok-pressure-test reviews/grok.md "$(_fill "$_DEVLOOP_TMPL/trigger-pressure.txt")"; then
    echo "grok-pressure-test OK"
  else echo "grok-pressure-test STALLED"; rc=1; fi
  return "$rc"
}

# devloop_escalate <task> <collect-output…> : human-readable escalation when a reviewer stalled.
...
# _devloop_set_ctx_impl <task> <iter> : impl/iteration context on top of _devloop_set_ctx. Run from the
...
_devloop_set_ctx_impl() {
  _devloop_set_ctx "$1"
  export DL_ITER="$2"
  export DL_WORKTREE_NAME="devloop-$1"
  ...
  # policy-checker agents) does NOT live here — it lives in the repo's .devloop/config as a full
  # DL_GATE_STEP/DL_GATE_CONTRACT override (see _devloop_load_config; e.g. the reference Rust consumer
  ...
  # that's why the prose says "then", never `&&`). _devloop_set_ctx empties unset values, so :- here
  ...
  # checkout off <base>" instruction. _devloop_set_ctx emptied them above, so a pre-set env value survives
  # and is honored here; otherwise the original defaults apply unchanged.
  if [ "$2" -le 0 ]; then
    # plan phase (iter 0, via devloop_dispatch_plan): fresh worktree, no feedback yet.
    export DL_WORKTREE_STEP="${DL_WORKTREE_STEP:-}"
    export DL_FEEDBACK_STEP="${DL_FEEDBACK_STEP:-}"
  elif [ "$2" -le 1 ]; then
...
# devloop_dispatch_impl <task> <iter> [brief] : write task.md + brief.txt (iter 1 only) + status.md,
...
devloop_dispatch_impl() {
  local task="$1" iter="$2" brief="${3:-}"
  _devloop_set_ctx_impl "$task" "$iter"
  ...
  if [ "$iter" = 1 ] && [ "${DL_PLAN_FIRST:-0}" = 1 ] && [ ! -f "$DL_BUSDIR/iter-0/plan.md" ]; then
    echo "dispatch_impl: DL_PLAN_FIRST=1 but no iter-0/plan.md — run devloop_dispatch_plan $task \"<brief>\" first" >&2; return 5
  fi
  ...
  if [ "$iter" -le 1 ]; then
    ...
    _fill "$_DEVLOOP_TMPL/task-impl.md.tmpl" | bus_write "$task" task.md >/dev/null
  ...
  if [ "$iter" -le 1 ]; then
    ...
    local snap; snap="$(devloop_panes)"                                   # one snapshot, then here-string (setpgrp-flake guard)
  ...
  if [ -z "$impl_trigger" ]; then
    ...
    # pane). Delivery uses devloop_send (bundled pane run): a shell has no bracketed-paste-pill, so the
    # _devloop_fire/send_prompt path (a Claude-TUI workaround) does not apply. The pane shell evaluates
    ...
    _fill "$_DEVLOOP_TMPL/$ptmpl" > "$DL_ITERDIR/impl-prompt.txt"
    ...
    if [ -n "$grok_model" ]; then
      ...
      local pretrust="bash \"$_DEVLOOP_LIB/forge-pretrust.sh\" \"$DL_REPO_PATH/$wt\" \"$DL_REPO_PATH\""
    ...
    local tguard="bash \"$_DEVLOOP_LIB/target-guard.sh\";"
    if [ "$fresh_wt" = 1 ]; then
      ...
      _fetch="$(_devloop_base_fetch "$DL_REPO_PATH" "$_base")"
    ...
    devloop_send "$iid" "$cmd" || { echo "dispatch_impl: $impl_label dispatch refused (workspace guard)" >&2; return 3; }
    echo "dispatched impl (guarded): $impl_label=$iid iter=$iter"
    return 0
  fi

  _devloop_fire "$iid" "$impl_label" "$(_fill "$_DEVLOOP_TMPL/$impl_trigger")" \
    || { echo "dispatch_impl: impl trigger not delivered (workspace guard or DEAD PANE — see stderr)" >&2; return 3; }
  echo "dispatched impl (guarded): $impl_label=$iid iter=$iter"
}

# devloop_dispatch_plan <task> "<brief>" : plan-first phase (DL_PLAN_FIRST, 2026-07-07, maintainer decision). Runs the
...
# human → human `touch <bus>/plan-approved.md` → devloop_dispatch_impl <task> 1 (no brief re-pass needed;
...
devloop_dispatch_plan() {
  ...
  devloop_dispatch_impl "$task" 0 "$brief"
}

# devloop_dispatch_review_iter <task> <iter> <worktree_path> : worktree-targeted two-lens review of the
...
# write-back contract. {{BRANCH}} is the WORKTREE's checked-out branch (worktree-devloop-<task>), read from
# the worktree itself — NOT the orchestrator's HEAD (main), which is what _devloop_set_ctx defaults to.
devloop_dispatch_review_iter() {
  ...
  export DL_WORKTREE_NAME="devloop-$task"
  ...
  _devloop_fire "$rid" claude-code-review      "$(_fill "$_DEVLOOP_TMPL/trigger-review-iter.txt")"   || { echo "review_iter: review trigger not delivered (workspace guard or DEAD PANE — see stderr)" >&2; return 3; }
  ...
  if [ "${DL_GROK_LENS:-pane}" = opencode ]; then
    echo "grok lens: opencode (tier=${DL_TIER:-default} -> $([ "${DL_TIER:-}" = tier1 ] && echo grok-4.20-multi-agent || echo grok-4.3))"
    bash "$_DEVLOOP_LIB/grok-review.sh" "$DL_TASK" "iter-$iter/reviews/grok.md" "$wt" "$DL_TARGET" \
      || { echo "review_iter: grok-review helper failed" >&2; return 1; }
  else
    _devloop_fire "$gid" grok-pressure-test "$(_fill "$_DEVLOOP_TMPL/trigger-pressure-iter.txt")" || { echo "review_iter: pressure trigger not delivered (workspace guard or DEAD PANE — see stderr)" >&2; return 3; }
  fi
  echo "dispatched review (guarded): claude-code-review=$rid grok-pressure-test=$gid iter=$iter target=$wt"
}

# devloop_collect_iter <task> <iter> <worktree_path> : DEADLINE-branch collect for the iteration review —
# reprompt-bounded per reviewer (reuses devloop_collect_one with iter-pathed relpaths). Re-establishes the
# worktree-targeted context so a reprompt on a fresh re-invoke re-fills identically. Echoes "<label> OK|STALLED".
devloop_collect_iter() {
  local task="$1" iter="$2" wt="$3" rc=0
  _devloop_set_ctx "$task"; export DL_REPO_PATH="$wt" DL_WORKTREE_NAME="devloop-$task"
  export DL_TARGET="branch $DL_WORKTREE_NAME vs $DL_WORKTREE_BASE (worktree $wt), review round $iter"
  export DL_ITERDIR; DL_ITERDIR="$(bus_iter_dir "$task" "$iter")"
  if devloop_collect_one "$task" claude-code-review "iter-$iter/reviews/claude.md" "$(_fill "$_DEVLOOP_TMPL/trigger-review-iter.txt")"; then
    echo "claude-code-review OK"; else echo "claude-code-review STALLED"; rc=1; fi
  if devloop_collect_one "$task" grok-pressure-test "iter-$iter/reviews/grok.md" "$(_fill "$_DEVLOOP_TMPL/trigger-pressure-iter.txt")"; then
    echo "grok-pressure-test OK"; else echo "grok-pressure-test STALLED"; rc=1; fi
  return "$rc"
}

# devloop_gate_verdict <task> <iter> : echo PASS|FAIL parsed from iter-<n>/gate.md's machine line
...
devloop_gate_verdict() {
  bus_ready "$1" "iter-$2/gate.md" || { echo "devloop_gate_verdict: iter-$2/gate.md not ready" >&2; return 1; }
  local v; v="$(bus_read "$1" "iter-$2/gate.md" | sed -n 's/^GATE:[[:space:]]*//p' | tail -n 1)"
  case "$v" in
    PASS|FAIL) printf '%s\n' "$v" ;;
    *) echo "devloop_gate_verdict: no GATE: PASS|FAIL line in iter-$2/gate.md" >&2; return 1 ;;
  esac
}

# devloop_dispatch_teardown <task> : fire the teardown trigger to the impl pane (guarded). Only call AFTER
...
devloop_dispatch_teardown() {
  ...
  local trig; trig="$(sed -e "s#{{TASK}}#${DL_TASK}#g" -e "s#{{BUSDIR}}#${DL_BUSDIR}#g" \
      ...
      "$_DEVLOOP_TMPL/trigger-teardown.txt")"
  # Find whichever impl pane ran this task (grok-headless/forge are shell arms — the default + escalation).
  local snap iid label; snap="$(devloop_panes)"                 # one snapshot + here-string (setpgrp-flake guard)
  ...
  if [ "$label" = forge-implementation ] || [ "$label" = grok-headless-implementation ]; then
    ...
    devloop_send "$iid" "cd \"$DL_REPO_PATH\" && git worktree remove \"$wt\" --force && git branch -D \"worktree-$DL_WORKTREE_NAME\"; mkdir -p \"$DL_ARCHIVE_DIR\" && mv \"$DL_BUSDIR\" \"$DL_ARCHIVE_DIR/$DL_TASK-done\"" \
      || { echo "teardown: shell teardown refused (workspace guard)" >&2; return 3; }
    echo "teardown dispatched (guarded, shell pane) for $task"
    return 0
  fi
  # _devloop_fire (send-text + separate Enter, verify pickup) — same reason the other fires use it:
  # the bundled `pane run` can collapse a longer trigger into an unsubmitted paste pill.
  _devloop_fire "$iid" "$label" "$trig" || { echo "teardown: trigger not delivered (workspace guard or DEAD PANE — see stderr)" >&2; return 3; }
