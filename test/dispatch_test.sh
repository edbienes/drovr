...
#   1. devloop_collect_iter's ready-path resolves both lenses to OK without touching panes
...
# Isolated bus so we never touch a real ~/.devloop room.
TMP="$(mktemp -d)"; export DEVLOOP_BUS_ROOT="$TMP" DEVLOOP_REPO_SLUG="dispatch-test"
trap 'rm -rf "$TMP"' EXIT

# --- 1. devloop_collect_iter ready-path: both lenses staged complete → both OK, exit 0, no panes ---
...
out="$(devloop_collect_iter "$TASK" 1 /tmp/fake-worktree 2>/dev/null)"; rc=$?
...
export DL_TASK=demo DL_REPO=super-school-rs DL_REPO_PATH=/tmp/wt DL_WORKTREE_NAME=devloop-demo
export DL_TARGET="branch devloop-demo vs main (worktree /tmp/wt), review round 2"
export DL_ITERDIR="$TMP/dispatch-test/demo/iter-2" DL_BUSDIR="$TMP/dispatch-test/demo"
trig="$(_fill "$_DEVLOOP_TMPL/trigger-review-iter.txt")"
...
# --- 3. unsubmitted-trigger guard: every trigger fire routes through _devloop_fire (confirm pickup +
#         re-deliver once), and trigger DELIVERY uses devloop_send_prompt (send-text + a SEPARATE Enter),
#         never the bundled `herdr pane run` (devloop_send) whose Enter is absorbed into a long-text
...
assert_eq "$(grep -c '^_devloop_fire()' "$SRC")" "1" "_devloop_fire helper is defined"
# The unsubmitted-trigger retry is a DISCRETE Enter (submits a stuck paste-pill, no-op on an empty
# composer), NOT a re-send of the whole prompt — a full re-send double-fired the LLM whenever _devloop_busy
# false-negatived a working agent (observed live 2026-06-25). Guard the bare-Enter retry against regressing.
assert_eq "$(grep -cE 'herdr pane send-keys .* Enter' "$SRC")" "1" "_devloop_fire retry submits via a discrete Enter (no prompt re-send)"
assert_eq "$(grep -c 'for attempt in 1 2' "$SRC")" "0" "_devloop_fire does not loop a full re-send (would double-fire the LLM)"
fires="$(grep -cE '^[[:space:]]*_devloop_fire \"' "$SRC")"
assert_eq "$([ "$fires" -ge 5 ] && echo ok)" "ok" "all trigger fires route through _devloop_fire (>=5: impl + 2 review-stage + 2 review-iter; found $fires)"
# both trigger-delivery sites (_devloop_fire + the collect reprompt) use the send-text+Enter primitive.
assert_eq "$(grep -cE '^[[:space:]]*devloop_send_prompt ' "$SRC")" "2" "trigger deliveries use devloop_send_prompt (_devloop_fire + collect reprompt)"
# and none deliver a trigger via the bundled devloop_send (trailing space excludes devloop_send_prompt).
assert_eq "$(grep -cE 'devloop_send .*_fill.*trigger-' "$SRC")" "0" "no trigger is delivered via bundled devloop_send/pane run"
...
#         expose the grok label/trigger arms. DL_WORKTREE_NAME=devloop-demo was exported in section 2. ---
assert_contains "$(_devloop_launch_for grok-implementation)" "--worktree=devloop-demo" "grok-implementation launch carries --worktree=<task>"
_devloop_set_ctx_impl demo 2 >/dev/null   # populate impl/iter env (FEEDBACK_STEP, GATE_STEP, ITERDIR, …)
gtrig="$(_fill "$_DEVLOOP_TMPL/trigger-impl-grok.txt")"
...
assert_eq "$(_devloop_launch_for grok-implementation | grep -c -- '--effort')" "0" "grok-implementation launch carries no --effort (no-op for grok-build / headless-only)"
assert_eq "$(grep -c 'impl_trigger=trigger-impl-grok.txt' "$SRC")" "1" "dispatch selects the grok trigger for DL_IMPL_AGENT=grok"
assert_eq "$(grep -cE 'grok-implementation\)' "$DIR/../lib/provision.sh")" "2" "provision has a grok-implementation launch arm (+ _devloop_alive case)"
...
assert_eq "$(grep -c 'grok-headless-implementation)' "$PROV")" "2" "provision has a grok-headless launch arm (+ _devloop_alive case)"
...
assert_contains "$(_devloop_launch_for grok-headless-implementation)" "cd " "grok-headless launch parks the shell at repo root (no resident agent)"
...
assert_eq "$([ ! -f "$_DEVLOOP_TMPL/prompt-impl-codex.txt" ] && echo ok)" "ok" "codex-named prompt template is gone (renamed prompt-impl-shell.txt)"
ctrig="$(_fill "$_DEVLOOP_TMPL/prompt-impl-shell.txt")"
...
timpl="$(_fill "$_DEVLOOP_TMPL/task-impl.md.tmpl")"
...
# --- 8. plan-first (DL_PLAN_FIRST, added 2026-07-07 per Ed): devloop_dispatch_plan runs a PLAN-ONLY phase
...
assert_eq "$(grep -c '^devloop_dispatch_plan()' "$SRC")" "1" "devloop_dispatch_plan is defined"
# 8a. resident arms (claude / legacy grok TUI) are unsupported for the plan phase
out="$(cd "$TMP" && DL_IMPL_AGENT=claude devloop_dispatch_plan planp "brief" 2>&1)"; rc=$?
...
ptrig="$(_fill "$_DEVLOOP_TMPL/prompt-impl-plan.txt")"
...
out="$(cd "$TMP" && DL_PLAN_FIRST=1 devloop_dispatch_impl planp 1 "brief" 2>&1)"; rc=$?
assert_eq "$rc" "5" "DL_PLAN_FIRST=1 without iter-0/plan.md refuses iter-1 (rc=5)"
assert_contains "$out" "devloop_dispatch_plan" "the refusal points at devloop_dispatch_plan"
# 8d. an existing but UNAPPROVED plan blocks iter-1 (the human pause is the point)
printf 'the plan\n' | bus_write planp "iter-0/plan.md" >/dev/null
out="$(cd "$TMP" && devloop_dispatch_impl planp 1 "brief" 2>&1)"; rc=$?
...
plan_ctx="$(cd "$TMP" && _devloop_set_ctx_impl planp 1 && printf 'F=%s\nW=%s\n' "$DL_FEEDBACK_STEP" "$DL_WORKTREE_STEP")"
...
# --- 10. per-repo .devloop/config (extraction P1, 2026-07-09): repo POLICY loads from
#          <repo>/.devloop/config in a sandboxed empty-env subshell, whitelisted DL_* only, precedence
...
out="$(unset DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       cd "$CFGREPO" && _devloop_set_ctx_impl cfg-none 1 >/dev/null 2>&1; printf '%s' "$DL_GATE_STEP")"
...
out="$(unset DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       export DL_GATE_PROFILE=python; cd "$CFGREPO" && _devloop_set_ctx_impl cfg-py 1 >/dev/null 2>&1; printf '%s' "$DL_GATE_STEP")"
...
printf 'DL_GATE_PROFILE=web\n' > "$CFGREPO/.devloop/config"
out="$(unset DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       cd "$CFGREPO" && _devloop_set_ctx_impl cfg-web 1 >/dev/null 2>&1; printf '%s' "$DL_GATE_STEP")"
...
out="$(unset DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       export DL_GATE_PROFILE=python; cd "$CFGREPO" && _devloop_set_ctx_impl cfg-envwin 1 >/dev/null 2>&1; printf '%s' "$DL_GATE_STEP")"
...
printf 'DL_GATE_PROFILE=web\nDL_TASK=evil\nDL_REPO_PATH=/pwned\n' > "$CFGREPO/.devloop/config"
out="$(unset DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       cd "$CFGREPO" && _devloop_set_ctx_impl cfg-sandbox 1 >/dev/null 2>&1; printf '%s|%s' "$DL_TASK" "$DL_REPO_PATH")"
...
rm -f "$CFGREPO/.devloop/config"
out="$(unset DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       cd "$CFGREPO" && _devloop_set_ctx_impl cfg-base 1 >/dev/null 2>&1; printf '%s' "$DL_WORKTREE_STEP")"
assert_contains "$out" "branched off origin/main" "worktree base defaults to origin/main"
printf 'DL_WORKTREE_BASE=origin/develop\n' > "$CFGREPO/.devloop/config"
out="$(unset DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       cd "$CFGREPO" && _devloop_set_ctx_impl cfg-base2 1 >/dev/null 2>&1; printf '%s' "$DL_WORKTREE_STEP")"
...
# 10f-2. _devloop_base_fetch: fetch prefix ONLY for a remote-qualified base — a slash-y LOCAL branch
#        (feat/x) must not be misread as remote+branch (found by the cortex P2 run, 2026-07-09).
assert_eq "$(_devloop_base_fetch "$CFGREPO" "feat/m8a-interactive-auth")" "" "slash-y local branch base gets NO fetch prefix (repo has no such remote)"
assert_eq "$(_devloop_base_fetch "$CFGREPO" "main")" "" "local no-slash base gets no fetch prefix"
git -C "$CFGREPO" remote add origin /nonexistent-remote 2>/dev/null
assert_eq "$(_devloop_base_fetch "$CFGREPO" "origin/develop")" "git fetch -q origin develop && " "remote-qualified base gets the fetch prefix"
assert_eq "$(_devloop_base_fetch "$CFGREPO" "feat/m8a-interactive-auth")" "" "slash-y local branch still no fetch even with a remote present"
# 10g. GOLDEN byte-identity: the super-school-rs fixture reproduces the pre-extraction rust gate exactly.
#      (The real file is <super-school-rs>/.devloop/config — keep the fixture a verbatim copy.)
cp "$DIR/fixtures/super-school-rs.devloop-config" "$CFGREPO/.devloop/config"
...
out="$(unset DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       cd "$CFGREPO" && _devloop_set_ctx_impl cfg-gold 1 >/dev/null 2>&1; printf '%s' "$DL_GATE_STEP")"
assert_eq "$out" "$GOLD_STEP" "super-school-rs config reproduces pre-extraction DL_GATE_STEP byte-for-byte"
out="$(unset DL_GATE_PROFILE DL_GATE_STEP DL_GATE_CONTRACT DL_WORKTREE_BASE DL_WORKTREE_STEP DL_FEEDBACK_STEP
       cd "$CFGREPO" && _devloop_set_ctx_impl cfg-gold2 1 >/dev/null 2>&1; printf '%s' "$DL_GATE_CONTRACT")"
assert_eq "$out" "$GOLD_CONTRACT" "super-school-rs config reproduces pre-extraction DL_GATE_CONTRACT byte-for-byte"

# --- 7. dead-pane FAIL-CLOSED (added 2026-07-06, per Ed): _devloop_fire refuses to deliver a trigger to a
#         pane with no live agent — a prompt typed into a bare shell was fired-and-forgotten TWICE live
#         (06-24, 07-06). Functional: stub _devloop_alive dead → rc=4 and NO delivery attempt; stub alive +
...
assert_eq "$(grep -c '^_devloop_alive()' "$PROV")" "1" "_devloop_alive predicate is defined in provision.sh"
_devloop_alive() { return 1; }                        # stub: pane is dead
sent=0; devloop_send_prompt() { sent=1; return 0; }   # record any attempted delivery
_devloop_fire "w0-9" claude-code-review "trigger text" 2>/dev/null; rc=$?
assert_eq "$rc" "4" "_devloop_fire fails CLOSED (rc=4) on a dead pane"
assert_eq "$sent" "0" "_devloop_fire never delivers the prompt to a dead pane"
_devloop_alive() { return 0; }                        # stub: agent alive…
_devloop_busy()  { return 0; }                        # …and picks up immediately
_devloop_fire "w0-9" claude-code-review "trigger text" 2>/dev/null; rc=$?
assert_eq "$rc" "0" "_devloop_fire happy path unchanged (alive agent, instant pickup)"
assert_eq "$sent" "1" "_devloop_fire delivered the prompt to the live pane"
