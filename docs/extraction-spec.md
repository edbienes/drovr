# Drovr Extraction — Portable, Per-Repo, Eventually Open-Source
...
**Goal:** drovr becomes a harness any repo can adopt by declaring its own policy, developed safely alongside live use, and hardened toward open-sourcing.
...
- **The repo IS the live install.** `~/.claude/skills/drovr` is simultaneously the git repo and the deployed skill every session sources. There is no build/install step. Head is `878c98a`; the plan-first wiring (`drovr_dispatch_plan`/`DL_PLAN_FIRST`, suite 118/118) sits **uncommitted** in the working tree (`SKILL.md`, `lib/dispatch.sh`, `lib/provision.sh`, `test/dispatch_test.sh` modified; `lib/target-guard.sh`, `templates/prompt-impl-plan.txt` untracked). No remote — local history only.
- **State is already per-repo:** bus at `~/.drovr/<repo-slug>/<task>/`, worktrees per repo, Hindsight banks cwd-derived.
...
**Per-repo declaration** (the policy), read from a config file in the target repo — proposed `.drovr/config` (shell-sourceable, keeping bash-native; TOML is ceremony we don't need):

```sh
# .drovr/config — this repo's drovr policy
...
Precedence: env var (per-dispatch intent) > `.drovr/config` (repo policy) > built-in default. Existing env-var behavior is unchanged — this is backward-compatible by construction; super-school-rs works identically before and after with an empty config.

The named profiles (`rust`, `web`, `python`) become *generic* — cargo fmt/clippy/test, pnpm lint/typecheck/test, ruff/mypy/pytest — and everything repo-specific (ADR checkers, OpenAPI drift oracle) moves into super-school-rs's own `.drovr/config` as `DL_GATE_EXTRA_STEPS` / `DL_POLICY_AGENTS`. The gate composes: profile steps + extra steps + policy-agent roster.
...
1. **Never develop in `~/.claude/skills/drovr`.** Development happens in a separate clone (`~/Projects/Development/drovr`). The live path becomes a deploy target that only ever fast-forwards.
2. **Deploy = fast-forward at a quiet point**: no dispatch in progress (check `~/.drovr/*/` for active tasks / running panes), suite green in the dev clone first.
3. **Backward compatibility is a hard gate** per deploy: super-school-rs dispatches must behave identically with no `.drovr/config` present. The existing test suite is the oracle; extraction adds config-precedence tests.
...
- **P1 — Dev clone + config seam:** clone to `~/Projects/Development/drovr`; implement `.drovr/config` loading with the precedence above; unhardcode worktree base; genericize the `rust` profile and move super-school-rs specifics into a `.drovr/config` committed to super-school-rs. Suite extended for precedence + config parsing. Deploy via fast-forward; verify one real super-school-rs dispatch behaves identically.
- **P2 — Second consumer (proof of generality):** adopt drovr in a second real repo. The designated forcing function is school-#2 (`../super-school-py`, python profile) but it is gated behind the registrar worklist + quarter dates; if an interim proof is wanted sooner, any existing non-Rust repo (e.g. a small web/Node project) exercising `web` or a new profile via pure config counts. One full dispatch → gate → two-lens → triage cycle in that repo = P2 done.
...
A repo adopts drovr by: installing the skill (user-scope), running herdr, and committing a `.drovr/config` + optional `.claude/agents/` policy roster. No drovr-core edits. Two dissimilar repos (Rust + non-Rust) demonstrably run the full cycle from pure config. Super-school-rs behavior byte-identical throughout.
